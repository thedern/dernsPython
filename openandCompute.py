...
for line in fhand:
        ...
        x = line.find('X-DSPAM-Confidence')  #use string's find method to look for the substring in quotes

        if x == 0:  #if return code is '0', meaning a match perform the next commands

                y = line.find(' ') #find the space in the substring
                num = line[y+1:]   #slice from 1 character after the space to the end of the string and save
